    ...
    if not np.any(diff < 0):
        ...
        if boolvec[-1]:
            offsets = np.concatenate(
                (np.atleast_1d(offsets), np.atleast_1d(len(boolvec) - 1)))
...
class PlaylistPanel(wx.Panel):
    ...
    def __init__(self, parent, master, **kwargs):
        ...
        top_sizer.AddMany(
            [(button, 1, wx.EXPAND)
                for button in [self.prevbutton, self.startbutton,
                               self.nextbutton]
             ]
        )
    ...
    def onRunTask(self, event=False):
        ...
        if self.master.current_task is not None:
            ...
            if running:
                ...
                self.parent.optionpanel.checkboxes[
                    'show_photodiode'].Enable(True)

            else:
                l, c = obj.stop
                # grey out the photodiode checkbox while the task is running
                self.parent.optionpanel.checkboxes[
                    'show_photodiode'].Enable(False)
...
class StatusPanel(wx.Panel):
    ...
    def __init__(self, parent, master):
        ...
        timelabel = wx.StaticText(
            self, -1, 'Remaining:', size=(80, -1), style=wx.ALIGN_RIGHT)
        self.time = wx.StaticText(
            self, -1, '', size=(60, -1), style=wx.ALIGN_LEFT)
        framelabel = wx.StaticText(
            self, -1, 'Scan:', size=(80, -1), style=wx.ALIGN_RIGHT)
        self.frame = wx.StaticText(
            self, -1, '', size=(60, -1), style=wx.ALIGN_LEFT)
        fpslabel = wx.StaticText(
            self, -1, 'Min FPS:', size=(80, -1), style=wx.ALIGN_RIGHT)
        self.fps = wx.StaticText(
            self, -1, '', size=(60, -1), style=wx.ALIGN_LEFT)
    ...
    def onUpdate(self):
        ...
        if task:
            ...
            self.progressbar.SetValue(
                100 * float(frame + 1) / self.totalframes)
...
class AdjustPanel(wx.Panel):
    ...
    def __init__(self, parent, master):
        ...
        labels = ['X', 'Y', 'Scale']
        stext = [wx.StaticText(self, -1, label, size=(60, -1))
                 for label in labels]
        ...
        for nn in attrnames:
            ...
            ctrl = wx.TextCtrl(
                self, size=(60, -1), value=str(round(ref.get(), 2)),
        ...
        for nn in attrnames:
            ...
            ctrl = wx.TextCtrl(
                self, size=(60, -1),
                value=str(round(ref.get(), 2)), style=wx.TE_PROCESS_ENTER
            )
            ctrl.Bind(wx.EVT_TEXT_ENTER, self.onText)
            ctrl.ref = ref
...
class LogPanel(wx.Panel):
    ...
    def __init__(self, parent, master):
        ...
        button_hsizer = wx.BoxSizer(wx.HORIZONTAL)
        button_hsizer.AddMany([(but, 1, wx.EXPAND)
                              for but in [self.plot_button, self.clear_logs]])
    ...
    def onDiagnosticPlot(self, event=None):
        ...
        # ax1.fill_between(range(nframes),
        #                  [0] * nframes, self.master.stimcanvas.frametimes)
        ...
        stimon, stimoff = get_onsets_and_offsets(
            self.master.stimcanvas.stimdraws)
        if any(stimon) and any(stimoff):
            for ii in xrange(stimon.size):
                if ii == 0:
                    ax1.axvspan(stimon[ii], stimoff[ii], alpha=0.5,
                                color='g', label='Stimulus draws')
                else:
                    ax1.axvspan(stimon[ii], stimoff[ii],
                                alpha=0.5, color='g', label='__nolegend__')
        photoon, photooff = get_onsets_and_offsets(
            self.master.stimcanvas.photodraws)
        if any(photoon) and any(photooff):
            for ii in xrange(photoon.size):
                if ii == 0:
                    ax1.axvspan(photoon[ii], photooff[ii], alpha=0.5,
                                color='b', label='Photodiode draws')
                else:
                    ax1.axvspan(photoon[ii], photooff[ii],
                                alpha=0.5, color='b', label='__nolegend__')

        alldrawon, alldrawoff = get_onsets_and_offsets(
            self.master.stimcanvas.alldraws)
        if any(alldrawon) and any(alldrawoff):
            for ii in xrange(alldrawon.size):
                if ii == 0:
                    ax1.axvspan(alldrawon[ii], alldrawoff[ii],
                                alpha=0.5, color='r', label='Full-frame draws')
                else:
                    ax1.axvspan(alldrawon[ii], alldrawoff[ii],
                                alpha=0.5, color='r', label='__nolegend__')

        ax1.fill_between(np.arange(nframes), min_ft, max_ft,
                         alpha=0.1, facecolor='k', edgecolor='None')
        ax1.plot(np.arange(nframes), frametimes, '-k',
                 alpha=0.3, label='Frame drawtime')
        ax1.plot(np.arange(nframes), mean_ft, '-k',
                 alpha=0.75, label='Mean drawtime', lw=2)
        ax1.axhline(y=self.master.min_delta_t * 1E-3, ls='--',
                    c='k', alpha=0.75, label='Theoretical')
...
class OptionPanel(wx.Panel):
    ...
    def __init__(self, parent, master):
        ...
        if self.master.on_top:
            self.master.stimframe.SetWindowStyle(
                self.stimframestyle | wx.STAY_ON_TOP)
            self.parent.SetWindowStyle(
                self.controlwindowstyle | wx.STAY_ON_TOP)
        if self.master.fullscreen:
            self.master.stimframe.ShowFullScreen(True, style=wx.FULLSCREEN_ALL)

        check_statbox = wx.StaticBox(
            self, wx.VERTICAL, label='Display options')

        # checkboxes
        # --------------------------------------------------------------
        attrnames = ['show_photodiode', 'show_crosshairs',
                     'show_preview', 'fullscreen', 'on_top', 'run_loop']
        names = ['Show photodiode', 'Show crosshairs', 'Show preview',
                 'Fullscreen', 'Always on top', 'Display loop']
        events = [self.onPhoto, self.onCross, self.onPreview,
                  self.onFullscreen, self.onTop, self.onDisplayloop]
        checkboxes = []
    ...
    def onFullscreen(self, event=None):
        ...
        self.master.stimframe.ShowFullScreen(
            not isfullscreen, style=wx.FULLSCREEN_ALL)
        # force a full re-draw
...
class ControlWindow(wx.Frame):
    ...
    def __init__(self, parent, master, **kwargs):
        ...
        self.previewcanvas = glc.PreviewCanvas(
            self, self.master.stimcanvas, size=(420, 560))
        ...
        middle.Add(self.previewcanvas, 1, wx.SHAPED | wx.ALL |
                   wx.ALIGN_TOP | wx.ALIGN_CENTER_HORIZONTAL, border=2)
        ...
        self.Bind(wx.EVT_MENU, self.playlistpanel.onRunTask,
                  id=eventIDs['run'])
        self.Bind(wx.EVT_MENU, self.adjustpanel.keyboardNudge,
                  id=eventIDs['nudge'])
        self.Bind(wx.EVT_MENU, self.optionpanel.onDisplayloop,
                  id=eventIDs['display'])
        self.Bind(wx.EVT_MENU, self.optionpanel.onFullscreen,
                  id=eventIDs['full'])
